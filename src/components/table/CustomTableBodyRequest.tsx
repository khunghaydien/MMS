...
const CustomTableBody = ({
  ...
  const editableTableCell = (
    ...
    const onChangeEditableTableCell = (
      ...
      const newTmpRow = {
        ...tmpRow,
        [field]: field === 'assignEffort' ? `${value}%` : value,
      }
      setTmpRow(newTmpRow)
    }
            ...
            <>
              ...
              >
                <InputDateFns
                  ...
                  minDate={assignStartDate || startDate}
                  maxDate={endDate}
                  value={formatAnyToDate(tmpRow[column.id])}
                  onChange={(value: Date | null) =>
                    onChangeEditableTableCell(value, column.id, tmpRow)
        ...
        <ConditionalRender
          ...
          fallback={
            <TableCell key={column.id} className={classes.nowrap}>
              ...
              {!!row['useDeleteIcon'] && !!row['isCanUpdateStartDate'] && (
  ...
  const [tmpRow, setTmpRow] = useState<any>(null)
  const handleClear = () => {
    setTmpRow(tmpRow)
    setIsRowEditing(false)
  }
  const isTotal = tmpRow?.isTotal
  const isChange = useMemo(() => {
    return JSON.stringify(tmpRow) != JSON.stringify(tmpRow)
  }, [tmpRow])
  useEffect(() => {
    if (isChange) {
          ...
          return (
            <TableRow
              ...
              className={clsx(
                ...
                row.isTotal ? classes.rowTotal : ''
              )}
            >
              {useCheckbox && (
                <TableCell padding="checkbox">
                  {row && row.isActiveCheckbox === true ? (
                    <Checkbox
              ...
              {columns.map((column: TableHeaderColumn, index: number) => {
                ...
                return (
                  <Fragment key={index}>
                    ...
                    {column.id === 'copy' && CopyTableCell(row)}
                    {column.id !== 'delete' &&
                      column.id !== 'copy' &&
                      commonTableCell(row, column, isHighlight, isWeekend)}
                  </Fragment>
                )
              })}
            </TableRow>
